<template>
  <header class="app-header">
    ...
    <div class="header-right">
      ...
      <DropdownMenu
          :options="aiOptions"
          :trigger-text="$t('header.ai.title')"
          trigger-class="header-btn"
          :show-selection="false"
          @select="$emit('ai-select', $event.value)"
      >
        <template #trigger>
          <span :title="$t('header.ai.title')" style="display:inline-flex;align-items:center;gap:6px;color:var(--theme-primary)">
            <svg class="icon" viewBox="0 0 24 24" width="18" height="18" aria-hidden="true">
              <path fill="currentColor" d="M12,2A2,2 0 0,1 14,4C14,4.74 13.6,5.39 13,5.73V7H14A7,7 0 0,1 21,14H22A1,1 0 0,1 23,15V18A1,1 0 0,1 22,19H21V20A2,2 0 0,1 19,22H5A2,2 0 0,1 3,20V19H2A1,1 0 0,1 1,18V15A1,1 0 0,1 2,14H3A7,7 0 0,1 10,7H11V5.73C10.4,5.39 10,4.74 10,4A2,2 0 0,1 12,2M7.5,13A2.5,2.5 0 0,0 5,15.5A2.5,2.5 0 0,0 7.5,18A2.5,2.5 0 0,0 10,15.5A2.5,2.5 0 0,0 7.5,13M16.5,13A2.5,2.5 0 0,0 14,15.5A2.5,2.5 0 0,0 16.5,18A2.5,2.5 0 0,0 19,15.5A2.5,2.5 0 0,0 16.5,13Z"/>
            </svg>
            <span>{{ $t('header.ai.title') }}</span>
      ...
      <!-- 语法指南 -->
      <button class="header-btn" @click="$emit('show-guide')" @mouseenter="$emit('preload-guide')">
        <svg viewBox="0 0 24 24" width="16" height="16">
      ...
      >
        <template #trigger>
          <span :title="$t('header.language')" style="display:inline-flex;align-items:center;gap:6px;color:var(--theme-primary)">
            ...
            <span style="font-size:13px;font-weight:600">{{ localeLabel }}</span>
            <svg class="dropdown-arrow" viewBox="0 0 24 24" width="14" height="14">
              <path fill="currentColor" d="M7,10L12,15L17,10H7Z"/>
            </svg>
          </span>
      ...
      <button
          ...
          @mouseenter="$emit('preload-settings')"
...
const aiOptions = computed(() => {
  ...
  return [
    { label: t('header.ai.polish'), value: 'polish' },
    { label: t('header.ai.generateTitle'), value: 'title' },
    { label: t('header.ai.generateSummary'), value: 'summary' },
    { label: t('header.ai.generateImage'), value: 'image' }
  ]
})
